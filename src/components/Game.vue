<template lang="">
    ...
    <section class='w-fit m-auto'>
        ...
        <div class="bg-neutral-900 rounded">
            ...
            <div v-for="(row, i) in grid" :key="i" class = 'flex'>
                <div @click = "grid[i][j] ^= 1" @mouseover = "interaction($event, i, j)"  v-for="(value, j) in row" :key="j"
                 class = 'cell border border-neutral-800 ' :class = "{'bg-neutral-200' : grid[i][j]}"></div>
            </div>       
        </div>
        ...
        <div id="controls" class="grid grid-flow-col gap-10 mt-4 grid-cols-6">
            ...
            <!-- <button @click='log(grid)'>log</button> -->
...
const rPentomino = () => {
    ...
    rPentominoData.forEach(coords => {
        grid.value[coords[0]][coords[1]] = 1
    });
}
